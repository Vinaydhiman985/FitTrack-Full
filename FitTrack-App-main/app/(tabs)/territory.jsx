...
// Approx area per cell in m²  (55m × 55m = 3025 m²)
...
export default function TerritoryScreen({ onNav }) {
  ...
  // ── Load saved territory from AsyncStorage ──────────────────────────────
  useEffect(() => {
    const load = async () => {
      try {
  ...
  // ── Save territory whenever it changes ──────────────────────────────────
  useEffect(() => {
    if (Object.keys(cells).length === 0) return;
    AsyncStorage.setItem(STORAGE_KEYS.TERRITORY, JSON.stringify(cells)).catch(() => {});
  }, [cells]);

  // ── GPS permission + initial position ───────────────────────────────────
  const requestPermission = async () => {
    const { status } = await Location.requestForegroundPermissionsAsync();
    if (status !== 'granted') {
      setErrorMsg('Location permission denied. Enable it in Settings to use the territory map.');
      return;
    }
    try {
  ...
  // ── Start walking + claiming territory ──────────────────────────────────
  const startTracking = useCallback(async () => {
    ...
    locationSubRef.current = await Location.watchPositionAsync(
      ...
      (loc) => {
        ...
        // Claim the cell you're standing in + immediate neighbours
    ...
    <View style={[styles.container, { backgroundColor: dark ? '#0D1117' : '#F7F7FA' }]}>
      ...
      <View style={styles.mapWrap}>
        <MapView
          ...
          mapType="standard"
        >
          {/* ── YOUR TERRITORY CELLS ── */}
          {Object.keys(cells).map(key => (
            <Polygon
              key={key}
              coordinates={gridToPolygon(key)}
              fillColor="rgba(244,98,31,0.35)"
      ...
      <View style={[styles.statsPanel, { backgroundColor: dark ? 'rgba(10,12,18,0.97)' : 'rgba(255,255,255,0.97)' }]}>
        ...
        <View style={styles.statsPanelTop}>
          <View>
            ...
            <Text style={[styles.statCells, { color: dark ? '#9BA1A6' : '#6B7280' }]}>
              {cellCount} cells claimed
            </Text>
          </View>

          <View style={styles.statMeta}>
            <View style={styles.statMetaRow}>
              <Text style={styles.statMetaIcon}>👟</Text>
              <Text style={[styles.statMetaVal, { color: dark ? '#F0F0F5' : '#141416' }]}>
                {(user?.totalSteps || 0).toLocaleString()}
              </Text>
              <Text style={[styles.statMetaLabel, { color: dark ? '#9BA1A6' : '#6B7280' }]}>steps</Text>
            </View>
            <View style={styles.statMetaRow}>
              <Text style={styles.statMetaIcon}>🗺️</Text>
              <Text style={[styles.statMetaVal, { color: dark ? '#F0F0F5' : '#141416' }]}>
                {(user?.distance || 0).toFixed(1)}
              </Text>
        ...
        <View style={[styles.progressBg, { backgroundColor: dark ? 'rgba(255,255,255,0.08)' : '#F3F4F6' }]}>
          <View style={[styles.progressFill, {
            width: cellCount === 0 ? '0%' : `${Math.min(100, (cellCount / 500) * 100).toFixed(1)}%`,
          }]} />
        </View>
        <Text style={[styles.progressHint, { color: dark ? '#6B7280' : '#9CA3AF' }]}>
          {cellCount}/500 cells to full bar · Keep walking!
        </Text>
        ...
        <View style={styles.btnRow}>
          ...
          >
            <Text style={styles.trackBtnText}>
              {isTracking ? '⏹ Stop Claiming' : '▶ Start Claiming'}
            </Text>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  retryBtn: {
    marginTop: 8, backgroundColor: '#F4621F',
  ...
  coordText:  { flex: 1, fontSize: 10, fontWeight: '500', fontVariant: ['tabular-nums'] },
  centerBtn: {
  ...
  playerPulse: {
    ...
    borderRadius: 24, backgroundColor: 'rgba(244,98,31,0.2)',
  },
  playerDot: {
  ...
  playerRing: {
    ...
    borderColor: '#F4621F', opacity: 0.6,
  },
  ...
  statMetaIcon: { fontSize: 14 },
  statMetaVal:  { fontSize: 16, fontWeight: '800' },
  statMetaLabel:{ fontSize: 11, fontWeight: '500' },

  progressBg: {
    height: 6, borderRadius: 999, overflow: 'hidden', marginBottom: 4,
  },
  progressFill: {
    height: '100%', backgroundColor: '#F4621F', borderRadius: 999,
  },
  progressHint: {
  ...
  walkBtn: {
    width: 48, backgroundColor: '#FFF1EB',
    borderRadius: 999, alignItems: 'center', justifyContent: 'center',
  },
  walkBtnText: { fontSize: 20 },
  clearBtn: {
    width: 48, backgroundColor: '#FEE2E2',
    borderRadius: 999, alignItems: 'center', justifyContent: 'center',
  },
